# Stage 10.5 Final Release: Motor Hardware - Theory, Practice & Failure Analysis (16kHz PWM Version)
...
* **Thông số tử huyệt:**
  ...
  * Nội trở khi mở ($R_{DS(ON)}$): `~25mΩ` (Lý thuyết) $\rightarrow$ Thực tế ở áp kích $3.3V$ và chịu hiệu ứng nhiệt độ ở $16kHz$, nội trở phình lên khoảng **`57.5mΩ`** ($0.0575\Omega$).
  * Tụ ký sinh Miller ($C_{rss}$): `50 pF`. (Cầu nối truyền xung dội ngược từ Drain về Gate).
  * Điện dung buồng lái Gate ($C_{iss}$): `630 pF`.
  * Thời gian ngắt ($t_{D(off)} + t_f$): `~29ns` (Tốc độ ngắt cực nhanh đẻ ra xung dội cực mạnh).
...
Khi MOSFET mở toang cửa bằng áp kích 3.3V từ STM32, kết hợp hiệu ứng giãn nở nhiệt độ do đóng cắt 16,000 lần/giây, nội trở thực tế $R_{DS(ON)} \approx 0.0575\Omega$. Dòng điện chạy qua động cơ:
$$I = \frac{V_{RMS}}{R_m} = \frac{4.6V}{2.3\Omega} = 2A$$

Điện áp rớt trên MOSFET (quyết định độ nóng):
$$V_{Drain} = I \times R_{DS(ON)} = 2A \times 0.0575\Omega = \mathbf{0.115V}$$
...
$$\Delta V_{GS} = (V_{Drain} - 0.115V) \cdot \frac{C_{rss}}{C_{iss} + C_{rss}}$$
...
## PHẦN 6: CẤU HÌNH TẦN SỐ PWM VÀ BẢO VỆ QUÁ ÁP (BẢN 16kHz)
...
**2. Động lực học Tần số PWM 16kHz (The Sweet-spot):**
* **Tần số (f):** `16000 Hz` | **Chu kỳ (T):** $62.5 \mu s$
* **Nội trở vận hành $R_{DS(on)}$:** $0.0575 \Omega$ *(Đã phình to thêm +15% do nhiệt độ)*
* **Sụt áp $V_{Drain}$:** $0.115V$ *(Tăng $15mV$ so với 4kHz, hoàn toàn không đáng kể)*
* **Phân tích Nhiệt độ MOSFET:**
  * Tổn hao tĩnh (Static Loss): $P_{cond} = 2^2 \times 0.0575 \times 0.8 = \mathbf{0.184W}$
  * Tổn hao chuyển mạch (Switching Loss): $P_{sw} \approx \mathbf{0.037W}$ (Tăng gấp 4 lần so với 4kHz).
  * Tổng tổn hao: $\sim \mathbf{0.221W}$
  * Nhiệt độ linh kiện tăng: Khoảng **$+22^\circ C$** so với môi trường (Môi trường $30^\circ C \rightarrow$ FET nóng $\sim 52^\circ C$). 
👉 **Hành động BẮT BUỘC:** Phải dán thêm tản nhiệt nhôm mini hoặc đảm bảo luồng gió cánh quạt thổi qua mạch để tránh lùi nhiệt (Thermal Runaway).
* **Lợi ích Tuyệt đối:** * Dòng điện là đường DC phẳng tuyệt đối, **cứu sống chổi than** của động cơ khỏi các cú sốc búa đập.
    * Động cơ vận hành im lặng (**Silent operation**), không còn tiếng rít chói tai.